 ...
 */
public class BCRFMIndex<A, S extends Text<A>> implements Index<A, S> {
    ...
    private static <A, S extends Text<A>> BCRSymbolSequence<A, Location<S>>[] composeB(final Alphabet<A> alphabet) {
    ...
    @Override
    public List<Location<S>> locate(final Text<A> query) {
    ...
    private void indexBuildingInitialization(Set<InsertionSequenceTracker> remaining, Set<InsertionSequenceTracker>[][] remainingPerSymbol) {
        for (final InsertionSequenceTracker tracker : remaining) {
            ...
            final int code = alphabet.toCode(tracker.bSymbol);
            remainingPerSymbol[0][code].add(tracker);
            symbolCounts[code]++;
        }
    }
        ...
        for (int symbolCode = 0; symbolCode < B.length; symbolCode++) {
            ...
            while (!trackerToProcess.isEmpty()) {
                ...
                if (!tracker.hasPrevious()) // we reached the end of the sequence.
                    ...
                    tracker.bSymbol = tracker.previousSymbol();
                    final int bSymbolCode = alphabet.toCode(tracker.bSymbol);
                    if (!tracker.iterator.hasPrevious())
                        B[symbolCode].insert(position, tracker.bSymbol, new Location<>(tracker.sequence, tracker.sequence.length()));
                    else if((tracker.iterator.nextPosition() & locationMask) == 0)
    ...
    public Text<A> toBWTSequence() {
        final RLEText<A> result = new RLEText<>(alphabet);
    ...
    private class InsertionSequenceTracker implements Comparable<InsertionSequenceTracker> {
        final S sequence;
        final SymbolIterator<A> iterator;
        ...
        private A previousSymbol() {
            if (iterator.hasPrevious()) {
                ...
                return alphabet.toSymbol(0); // we return the first alphabet symbol as a placeholder for the sentinel.
            }
        }
