...
public final class ParamUtils {
    ...
    public static void requiresValidIndexInterval(final int from, final int to, final int length) {
        if (from < 0)
            throw new IllegalArgumentException("the from index cannot be negative");
        else if (from > to)
            throw new IllegalArgumentException("the from index cannot be greater than the to index");
        else if (to > length)
            throw new IllegalArgumentException("the to index cannot be greater than the length");
    }
}
